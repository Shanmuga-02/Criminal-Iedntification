...
def index():
    ...
    mycursor.execute("SELECT * FROM cf_camera order by rand() limit 0,1")
...
def add_station():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("SELECT count(*) FROM cf_policestation where station_id=%s",(station_id,))
...
def add_police():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("SELECT count(*) FROM cf_police where police_id=%s",(police_id,))
        cnt = mycursor.fetchone()[0]
        if cnt==0:
